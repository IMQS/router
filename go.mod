module github.com/IMQS/router

require (
	github.com/IMQS/go-apachelog v0.0.0-20180717125434-05952ec2100f
	github.com/IMQS/gowinsvc v0.0.0-20171019081213-88eed8ddfe95
	github.com/IMQS/httpbridge v0.0.0-20181012064022-ee7fa39680db
	github.com/IMQS/log v0.0.0-20190610134251-f890260a8190
	github.com/IMQS/serviceauth v0.0.0-20190709134506-cdd809512e5f
	github.com/IMQS/serviceconfigsgo v0.0.0-20181011085921-596c4358dcd4
	github.com/google/flatbuffers v1.11.0 // indirect
	golang.org/x/net v0.0.0-20190628185345-da137c7871d7
	gopkg.in/natefinch/lumberjack.v2 v2.0.0
)
