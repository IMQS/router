...
case ARGV[0]
	when "prepare"	then
		exec_or_die( "go build github.com/IMQS/router" )
	when "copy_out"	then
		FileUtils.cp( "router.exe", out_dir + '/bin/imqsrouter.exe' )
	when "test_unit" then
		...
		exec_or_die( "go test github.com/IMQS/router/server -test.cpu 2" )
